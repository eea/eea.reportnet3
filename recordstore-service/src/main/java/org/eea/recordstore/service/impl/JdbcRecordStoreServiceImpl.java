 ...
 */
@Service("jdbcRecordStoreServiceImpl") public class JdbcRecordStoreServiceImpl
    implements RecordStoreService {

    /**
     * The service instance id.
     */
    @Value("${spring.cloud.consul.discovery.instanceId}") private String serviceInstanceId;
     ...
     * The user postgre db.
     */
    @Value("${spring.datasource.dataset.username}") private String userPostgreDb;

    /**
     * The pass postgre db.
     */
    @Value("${spring.datasource.dataset.password}") private String passPostgreDb;

    /**
     * The conn string postgre.
     */
    @Value("${spring.datasource.url}") private String connStringPostgre;

    /**
     * The sql get datasets name.
     */
    @Value("${sqlGetAllDatasetsName}") private String sqlGetDatasetsName;

    /**
     * The dataset users.
     */
    @Value("${dataset.users}") private String datasetUsers;

    /**
     * The resource file.
     */
    @Value("classpath:datasetInitCommands.txt") private Resource resourceFile;

    /**
     * The resource file.
     */
    @Value("classpath:datasetInitCommandsCitusComplete.txt") private Resource resourceCitusFile;

    /**
     * The resource file.
     */
    @Value("classpath:datasetDistributeCitus.txt") private Resource resourceDistributeFile;

    /**
     * The resource file.
     */
    @Value("classpath:datasetInitCommandsCitus.txt") private Resource resourceDistributeFirstFile;


    /**
     * The path snapshot.
     */
    @Value("${pathSnapshot}") private String pathSnapshot;

    /**
     * The path snapshot disabled.
     */
    @Value("${pathSnapshotDisabled}") private String pathSnapshotDisabled;

    /**
     * The time to wait before releasing notification.
     */
    @Value("${dataset.creation.notification.ms}") private Long
        timeToWaitBeforeReleasingNotification;

    /**
     * The buffer file.
     */
    @Value("${snapshot.bufferSize}") private Integer bufferFile;

    /**
     * The batch distribute dataset.
     */
    @Value("${batchDistributeDataset}") private Integer batchDistributeDataset;
     ...
     */
    @Autowired private KafkaSenderUtils kafkaSenderUtils;

    /**
     * The data collection controller zuul.
     */
    @Autowired private DataCollectionControllerZuul dataCollectionControllerZuul;

    /**
     * The data set snapshot controller zuul.
     */
    @Autowired private DataSetSnapshotControllerZuul dataSetSnapshotControllerZuul;

    /**
     * The dataset controller zuul.
     */
    @Autowired private DataSetControllerZuul datasetControllerZuul;

    /**
     * The data set metabase controller zuul.
     */
    @Autowired private DataSetMetabaseControllerZuul dataSetMetabaseControllerZuul;

    /**
     * The dataset schema controller.
     */
    @Autowired private DatasetSchemaControllerZuul datasetSchemaController;

    /**
     * The eu dataset controller zuul.
     */
    @Autowired private EUDatasetControllerZuul euDatasetControllerZuul;

    /**
     * The test dataset controller zuul.
     */
    @Autowired private TestDatasetControllerZuul testDatasetControllerZuul;

    /**
     * The document controller zuul.
     */
    @Autowired private DocumentControllerZuul documentControllerZuul;

    /**
     * The dataflow controller zuul.
     */
    @Autowired private DataFlowControllerZuul dataflowControllerZuul;

    /**
     * The reference dataset controller zuul.
     */
    @Autowired private ReferenceDatasetControllerZuul referenceDatasetControllerZuul;

    /**
     * The validation controller zuul
     */
    @Autowired private ValidationControllerZuul validationControllerZuul;
     ...
     */
    @Override @Async public void createSchemas(Map<Long, String> datasetIdsAndSchemaIds,
        Long dataflowId, boolean isCreation, boolean isMaterialized) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            while ((command = br.readLine()) != null) {
                for (Long datasetId : datasetIdsAndSchemaIds.keySet()) {
                    statement.addBatch(command.replace("%dataset_name%",
                            LiteralConstants.DATASET_PREFIX + datasetId)
            ...
            // Release the lock
            String methodSignature = isCreation ?
            ...
            LOG.error("Unexpected error! Error in createSchemas for dataflowId {}. Message: {}",
     ...
     */
    @Override @Async public void distributeTables(Long datasetId) {

        // Initialize resources
        try (Connection connection = dataSource.getConnection();
            Statement statement = connection.createStatement();
            BufferedReader br = new BufferedReader(
                new InputStreamReader(resourceDistributeFile.getInputStream()))) {

            final List<String> citusCommands = new ArrayList<>();
            br.lines().forEach(citusCommands::add);

            for (String citusCommand : citusCommands) {
                citusCommand = citusCommand.replace("%dataset_name%",
                    LiteralConstants.DATASET_PREFIX + datasetId);
                jdbcTemplate.execute(citusCommand);
            }
        } catch (final IOException | SQLException e) {
            LOG_ERROR.error("Error reading commands file to distribute the dataset {}. {}",
                datasetId, e.getMessage());
            try {
                throw new RecordStoreAccessException(
                    String.format("Error reading commands file to distribute the dataset. %s",
                        e.getMessage()), e);
            } catch (RecordStoreAccessException e1) {
                LOG.info(e1.getMessage(), e);
            }
        } catch (Exception e) {
            LOG.error("Unexpected error! Error in distributeTables for datasetId {}. Message: {}",
                datasetId, e.getMessage());
            throw e;
        }
    }
     ...
     * @param datasetId the dataset id
     */
    @Override public void distributeTablesJob(Long datasetId) {

        // Initialize resources
        try (Connection connection = dataSource.getConnection();
            Statement statement = connection.createStatement();
            BufferedReader br = new BufferedReader(
                new InputStreamReader(resourceCitusFile.getInputStream()))) {
            ...
            LOG.error(
                "Unexpected error! Error in distributeTablesJob for datasetId {}. Message: {}",
                datasetId, e.getMessage());
            throw e;
        }
     ...
     */
    @Override public List<String> getNotdistributedDatasets() {
     ...
     */
    @Override @Async public void createEmptyDataSet(String datasetName, String idDatasetSchema)
        ...
        try (BufferedReader br = new BufferedReader(
            ...
            LOG_ERROR.error("Error reading commands file to create the idDatasetSchema {}. {}",
                idDatasetSchema, e.getMessage());
            throw new RecordStoreAccessException(
                String.format("Error reading commands file to create the dataset. %s",
                    e.getMessage()), e);
        } catch (Exception e) {
            LOG.error(
                "Unexpected error! Error in createEmptyDataSet for datasetSchemaId {}. Message: {}",
                idDatasetSchema, e.getMessage());
            throw e;
        }
        ...
        jdbcTemplate.execute(
            String.format(GRANT_ALL_PRIVILEGES_ON_SCHEMA, datasetName, datasetUsers));
        jdbcTemplate.execute(
        ...
        if (StringUtils.isNotBlank(datasetName) && StringUtils.isNotBlank(idDatasetSchema)) {
            ...
            LOG.info("DS created with the id {} and idDatasetSchema {}", idDataset,
                idDatasetSchema);
        }
    }
     ...
     */
    @Override public ConnectionDataVO getConnectionDataForDataset(String datasetName) {
     ...
     * @return the connection data for dataset
     */
    @Override public List<ConnectionDataVO> getConnectionDataForDataset() {
     ...
     */
    @Override @Async public void createDataSnapshot(Long idDataset, Long idSnapshot,
        Long idPartitionDataset, String dateRelease, boolean prefillingReference)
        ...
        try (Connection con = DriverManager.getConnection(connectionDataVO.getConnectionString(),
            ...
            String copyQueryTable =
                "COPY (SELECT id, id_table_schema, dataset_id FROM dataset_" + idDataset
                    + ".table_value) to STDOUT";
            ...
            if (DatasetTypeEnum.COLLECTION.equals(typeDataset)) {
                ...
                copyQueryRecord =
                    "COPY (SELECT id, id_record_schema, id_table, " + idPartitionDataset
                ...
                copyQueryField =
                    "COPY (SELECT fv.id, fv.type, fv.value, fv.id_field_schema, fv.id_record from dataset_"
                        ...
                        + ".record_value rv WHERE fv.id_record = rv.id "
                ...
                copyQueryRecord =
                    "COPY (SELECT id, id_record_schema, id_table, dataset_partition_id, data_provider_code FROM dataset_"
                        + idDataset + ".record_value WHERE dataset_partition_id="
                        + idPartitionDataset + " order by data_position) to STDOUT";
                copyQueryField =
                    "COPY (SELECT fv.id, fv.type, fv.value, fv.id_field_schema, fv.id_record from dataset_"
                ...
                while ((System.currentTimeMillis() - startTime) < 30000) {
                    try {
                        ...
                        LOG.info(
                            ...
                            nameFileRules, idDataset, idSnapshot,
                            timeToWaitBeforeReleasingNotification);
                        Thread.sleep(timeToWaitBeforeReleasingNotification);
                    }
                }
            ...
            if (SNAPSHOT.equals(type)) {
                ...
                createSnapshot.put(LiteralConstants.SIGNATURE,
                    LockSignature.CREATE_SNAPSHOT.getValue());
     ...
     */
    @Override @Async public void createSnapshotToClone(Long originDataset, Long targetDataset,
        ...
        String nameFileTableValue =
            pathSnapshot + String.format(FILE_CLONE_PATTERN_NAME, originDataset, targetDataset,
                LiteralConstants.SNAPSHOT_FILE_TABLE_SUFFIX);
        ...
        try (Connection con = DriverManager.getConnection(connectionDataVO.getConnectionString(),
            ...
            String copyQueryTable =
                "COPY (SELECT id, id_table_schema, dataset_id FROM dataset_" + originDataset
                    + ".table_value) to STDOUT";
            String copyQueryRecord =
                "COPY (SELECT rv.id, rv.id_record_schema, rv.id_table, " + partitionDatasetTarget
                    + ", rv.data_provider_code FROM dataset_" + originDataset
                    + ".record_value rv, dataset_" + originDataset + ".table_value tv "
                    + "WHERE rv.id_table=tv.id and tv.id_table_schema in (" + placeHolders
                    + ")) to STDOUT";
            String copyQueryField =
                "COPY (SELECT fv.id, fv.type, fv.value, fv.id_field_schema, fv.id_record from dataset_"
                    + originDataset + ".field_value fv, dataset_" + originDataset
                    + ".record_value rv, dataset_" + originDataset + ".table_value tv"
                    + " WHERE fv.id_record = rv.id and rv.id_table=tv.id"
            ...
            modifySnapshotFile(dictionaryOriginTargetObjectId,
                Arrays.asList(nameFileTableValue, nameFileRecordValue, nameFileFieldValue,
                    nameFileAttachmentValue), targetDataset);
            ...
            LOG.error(
                "Unexpected error! Error in createSnapshotToClone in print for originDataset {} and targetDataset {}. Message: {}",
                originDataset, targetDataset, e.getMessage());
            throw e;
        }
        ...
        try (Connection con = DriverManager.getConnection(conexion.getConnectionString(),
            conexion.getUser(), conexion.getPassword()); Statement stmt = con.createStatement()) {
            ...
            String copyQueryTable = COPY_DATASET + targetDataset
            ...
            LOG_ERROR.error("Error restoring the data into the target dataset {}", targetDataset,
                e);
        } catch (Exception e) {
            LOG.error(
                "Unexpected error! Error in createSnapshotToClone in copy for originDataset {} and targetDataset {}. Message: {}",
                originDataset, targetDataset, e.getMessage());
            throw e;
        } finally {
            ...
            LOG.info(
                "Process copying the data prefilled from the dataset {} to dataset {} finished",
                originDataset, targetDataset);
        }
    }
     ...
     * @throws IOException  Signals that an I/O exception has occurred.
     */
    @Override public void restoreDataSnapshot(Long idReportingDataset, Long idSnapshot,
        Long partitionId, DatasetTypeEnum datasetType, Boolean isSchemaSnapshot, Boolean deleteData,
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    @Override public void deleteDataSnapshot(Long idReportingDataset, Long idSnapshot)
        throws IOException {
     ...
     */
    @Override public void deleteDataset(String datasetSchemaName) {
     ...
     */
    @Override public void createDataSetFromOther(String sourceDatasetName,
     ...
     * @throws RecordStoreAccessException the record store access exception
     */
    @Override public void executeQueryViewCommands(String command)
        throws RecordStoreAccessException {
        jdbcTemplate.execute(command);
     ...
     * @param isMaterialized the is materialized
     */
    @Override public void createUpdateQueryView(Long datasetId, boolean isMaterialized) {
        ...
        try {
            ...
            LOG_ERROR.error("Error deleting Query view for datasetId {} : {}", datasetId,
                e1.getMessage(), e1);
        }
        ...
        datasetSchema.getTableSchemas().stream()
            ...
            .forEach(table -> {
                ...
                try {
                    ...
                    LOG_ERROR.error("Error creating Query view for datasetId {}: {}", datasetId,
                        e.getMessage(), e);
                }
            });
    }
     ...
     * @param isMaterialized the is materialized
     */
    @Async @Override public void createUpdateQueryViewAsync(Long datasetId,
     ...
     */
    @Override @Async public void updateMaterializedQueryView(Long datasetId, String user,
        Boolean released, String processId) {
        ...
        try {
            ...
            LOG_ERROR.error("Error updating Materialized view for datasetId {} : {}", datasetId,
                e.getMessage(), e);
        }
     ...
     * @throws RecordStoreAccessException the record store access exception
     */
    @Override public void launchUpdateMaterializedQueryView(Long datasetId)
        throws RecordStoreAccessException {
     ...
     * @param processId          the process id
     */
    @Override @Async public void refreshMaterializedQuery(List<Long> datasetIds,
        boolean continueValidation, boolean released, Long datasetId, String processId) {
        datasetIds.forEach(id -> {
            if (!Boolean.TRUE.equals(datasetControllerZuul.getCheckView(id))) {
                ...
                LOG.info("These materialized views: {} have been refreshed for datasetId {}.",
                    viewList, datasetId);
            } else {
     ...
     * @param datasetId the dataset id
     */
    @Override public void updateSnapshotDisabled(Long datasetId) {
        ...
        for (File file : matchingFilesToMove) {
            if (file.renameTo(new File(pathSnapshotDisabled + file.getName()))) {
                LOG.info("File: {} moved to: {} for datasetId {}", file.getName(),
        ...
        if (!CollectionUtils.isEmpty(nameFiles)) {
            nameFiles.stream().forEach(f -> {
                ...
                try (Stream<String> lines = Files.lines(pathFile)) {
                    replaced = lines.map(line -> line =
                            modifyingLine(dictionaryOriginTargetObjectId, line, f, datasetId))
                        .collect(Collectors.toList());
                    Files.write(pathFile, replaced);
                } catch (IOException e) {
                    LOG_ERROR.error(
                        "Error modifying the file {} during the data copy in cloning process for datasetId {}",
                        f, datasetId);
                } catch (Exception e) {
                    LOG.error(
                        "Unexpected error! Error in modifySnapshotFile for datasetId {}. Message: {}",
                        datasetId, e.getMessage());
                    throw e;
                }
            });
    ...
    private String modifyingLine(Map<String, String> dictionaryOriginTargetObjectId, String line,
        ...
        if (fileName.contains("TableValue")) {
            ...
            if (null != dictionaryOriginTargetObjectId) {
                line =
                    line.replace(tableSchemaId, dictionaryOriginTargetObjectId.get(tableSchemaId));
            }
        }
        return line;
    }
        ...
        if (!CollectionUtils.isEmpty(fileNames)) {
            fileNames.stream().forEach(f -> {
                try {
                    ...
                    LOG_ERROR.error(
                        "Error deleting the file {} during the data copy in cloning process", f);
                }
            });
        }
    }
    ...
    private void removeLocksRelatedToPopulateEU(Long dataflowId) {
        ...
        populateEuDataset.put(LiteralConstants.SIGNATURE,
            LockSignature.POPULATE_EU_DATASET.getValue());
        ...
        for (ReportingDatasetVO reporting : reportings) {
            ...
            relaseSnapshots.put(LiteralConstants.SIGNATURE,
                LockSignature.RELEASE_SNAPSHOTS.getValue());
    ...
    private String checkType(Long idDataset, Long idSnapshot) {
        ...
        if (snapshot != null) {
            ...
            if (DatasetTypeEnum.COLLECTION.equals(
                dataSetMetabaseControllerZuul.getType(idDataset))) {
                type = COLLECTION;
            }
        }
        return type;
        ...
        switch (type) {
            case SNAPSHOT:
                ...
                if (Boolean.TRUE.equals(snapshot.getRelease())) {
                    ...
                    releaseNotificableKafkaEvent(EventType.ADD_DATASET_SNAPSHOT_COMPLETED_EVENT,
                        value, idDataset, null);
                }
                break;
            case COLLECTION:
                ...
                kafkaSenderUtils.releaseKafkaEvent(
                    EventType.ADD_DATACOLLECTION_SNAPSHOT_COMPLETED_EVENT, valueEU);
                break;
            case REFERENCE:
                ...
                break;
            case SCHEMA:
                releaseNotificableKafkaEvent(EventType.ADD_DATASET_SCHEMA_SNAPSHOT_COMPLETED_EVENT,
                    value, idDataset, null);
                break;
            default:
                break;
    ...
    private void restoreSnapshot(Long datasetId, Long idSnapshot, Long partitionId,
        ...
        try (Connection con = DriverManager.getConnection(conexion.getConnectionString(),
            ...
            if (Boolean.TRUE.equals(deleteData) && !DatasetTypeEnum.EUDATASET.equals(datasetType)
                ...
                LOG.info("Deleting previous data for snapshotId {} and datasetId {}", idSnapshot,
                    datasetId);
                stmt.executeUpdate(sql);
            } else if (Boolean.TRUE.equals(deleteData) && DatasetTypeEnum.EUDATASET.equals(
                datasetType)) {
                ...
                LOG.info("Deleting previous data of the providers {} in the EU dataset {}",
                    providersCode, datasetId);
                stmt.executeUpdate(sql);

                // Delete the temporary etlExport table
                String sqlDeleteTempEtlExport =
                    "truncate table dataset_" + datasetId + ".temp_etlexport";
            ...
            DataSetMetabaseVO dataset =
                dataSetMetabaseControllerZuul.findDatasetMetabaseById(datasetId);
            LOG.info("Init restoring the snapshot files from Snapshot {} and datasetId {}",
                idSnapshot, datasetId);
            copyProcess(dataset.getDataflowId(), datasetId, idSnapshot, datasetType, cm);
            LOG.info("Finished restoring the snapshot files from Snapshot {} and datasetId {}",
                idSnapshot, datasetId);

            if (!DatasetTypeEnum.EUDATASET.equals(datasetType) && !successEventType.equals(
                EventType.RELEASE_COMPLETED_EVENT) && !prefillingReference) {
            ...
            if (DatasetTypeEnum.EUDATASET.equals(datasetType)) {
                ...
                List<DataCollectionVO> dcs =
                    dataCollectionControllerZuul.findDataCollectionIdByDataflowId(
                            ds.getDataflowId()).stream()
                        .sorted(Comparator.comparing(DataCollectionVO::getId))
                        .collect(Collectors.toList());
                List<Long> idsDc =
                    dcs.stream().sorted(Comparator.comparing(DataCollectionVO::getId))
                        .map(DataCollectionVO::getId).collect(Collectors.toList());

                if (!CollectionUtils.isEmpty(idsDc) && datasetIdFromSnapshot.equals(
                    idsDc.get(idsDc.size() - 1))) {
            ...
            if (!prefillingReference) {
                ...
                LOG_ERROR.error("Error restoring the snapshot data due to error {}.",
                    e.getMessage(), e);
     ...
     * @throws SQLException the SQL exception
     */
    private void copyProcess(Long dataflowId, Long datasetId, Long idSnapshot,
        DatasetTypeEnum datasetType, CopyManager cm) throws IOException, SQLException {
        try {
            if (DatasetTypeEnum.DESIGN.equals(datasetType) || DatasetTypeEnum.REFERENCE.equals(
                datasetType)) {
                ...
                LOG.info(
                    "Modified the file {} during the data copy in cloning process for datasetId {}",
                    nameFileTableValue, datasetId);

                String copyQueryTable = COPY_DATASET + datasetId
                    + ".table_value(id, id_table_schema, dataset_id) FROM STDIN";
                copyFromFile(copyQueryTable, nameFileTableValue, cm);
                LOG.info("Executed copyFromFile for table_value with file {} and datasetId {}",
                    nameFileTableValue, datasetId);
            }
            // Record value
            String nameFileRecordValue = pathSnapshot + String.format(FILE_PATTERN_NAME, idSnapshot,
            ...
            String copyQueryRecord = COPY_DATASET + datasetId
                + ".record_value(id, id_record_schema, id_table, dataset_partition_id, data_provider_code) FROM STDIN";
            copyFromFile(copyQueryRecord, nameFileRecordValue, cm);
            LOG.info("Executed copyFromFile for record_value with file {} and datasetId {}",
                nameFileRecordValue, datasetId);
            ...
            if (snapFileForSplitting.isForSplitting() == true) {
                ...
                for (int i = 1; i <= snapFileForSplitting.getNumberOfFiles(); i++) {
                    String splitFileName = String.format(SPLIT_FILE_PATTERN_NAME, idSnapshot, i,
                        LiteralConstants.SNAPSHOT_FILE_FIELD_SUFFIX);
                    ...
                    try {
                        ...
                        try {
                            ...
                            LOG.error(
                                "Error while trying to delete split snap file {} for dataflow {} and datasetId {}",
                                splitFileName, dataflowId, datasetId);
                        }
                    } catch (Exception e) {
            ...
            String copyQueryAttachment = COPY_DATASET + datasetId
                + ".attachment_value(id, file_name, content, field_value_id) FROM STDIN";
            copyFromFile(copyQueryAttachment, nameFileAttachmentValue, cm);
            LOG.info("Executed copyFromFile for attachment_value with file {} and datasetId {}",
                nameFileAttachmentValue, datasetId);
        } catch (Exception e) {
            LOG_ERROR.error(
                "Unexpected error! Error in copyProcess for datasetId {} and snapshotId {}. Message: {}",
                datasetId, idSnapshot, e.getMessage());
            throw e;
        }
    }
    ...
    void releaseNotificableKafkaEvent(EventType event, Map<String, Object> value, Long datasetId,
        ...
        if (!EventType.RELEASE_COMPLETED_EVENT.equals(event)) {
            try {
                ...
                LOG.error("Error releasing event {} for datasetId {} due to error {}", event,
    ...
    private List<String> getAllDataSetsName(String datasetName) {
        ...
        return jdbcTemplate.query(sqlGetDatasetsName, new PreparedStatementSetter() {
            ...
            @Override public List<String> extractData(ResultSet resultSet) throws SQLException {
    ...
    private void printToFile(String fileName, String query, CopyManager copyManager)
        ...
        try (OutputStream to = new FileOutputStream(fileName)) {
            ...
            LOG.error(
                "Unexpected error! Error in printToFile for fileName {} and query {}. Message: {}",
                fileName, query, e.getMessage());
            throw e;
        } finally {
            if (copyOut.isActive()) {
                copyOut.cancelCopy();
    ...
    private void copyFromFile(String query, String fileName, CopyManager copyManager)
        ...
        try (FileReader from = new FileReader(path.toString())) {
            ...
            LOG_ERROR.error(
                "Unexpected error! Error copying from file {} with query {} Message: {}", fileName,
                query, e.getMessage());
            throw e;
        }
    }
    ...
    private void copyFromFileRecovery(String query, String fileName, CopyManager copyManager)
        ...
        try {
            ...
            LOG_ERROR.error("Error in recover copy field process for fileName {} with error",
                fileName, e);
            throw e;
        }
    }
    ...
    private void splitSnapFile(String processId, String inputfile, Long idSnapshot,
        ...
        LOG.info(
            "Method splitSnapFile starts for file {} with idSnapshot {} and snapFileForSplitting {}",
            inputfile, idSnapshot, snapFileForSplitting);
    ...
    private SplitSnapfile isSnapFileForSplitting(String inputfile) {
        ...
        try {
            ...
            LOG.info("Method isSnapFileForSplitting ends for file {} with {} lines into {} files",
                inputfile, numberOfLines, numberOfFiles);
        } catch (Exception e) {
            LOG_ERROR.error("Error in file {} with error", inputfile, e);
        }
    ...
    private void createIndexMaterializedView(Long datasetId, String tableName)
        throws RecordStoreAccessException {
        String indexQuery =
            " CREATE UNIQUE INDEX " + "\"" + tableName + "_index" + "\"" + " ON dataset_"
                + datasetId + "." + "\"" + tableName + "\"" + " (record_id) ";
    ...
    private void executeViewPermissions(String queryViewName, Long datasetId)
        ...
        String querySelectPermission =
            "GRANT SELECT ON dataset_" + datasetId + "." + "\"" + queryViewName + "\"" + " TO "
                + datasetUsers;
        executeQueryViewCommands(querySelectPermission.toLowerCase());

        String queryDeletePermission =
            "GRANT DELETE ON dataset_" + datasetId + "." + "\"" + queryViewName + "\"" + " TO "
                + userPostgreDb;
    ...
    private void executeViewQuery(List<FieldSchemaVO> columns, String queryViewName,
        ...
        if (isMaterialized) {
            stringQuery.append(
                "CREATE MATERIALIZED VIEW if not exists dataset_" + datasetId + "." + "\""
                    + queryViewName + "\"" + " as (select rv.id as record_id, ");
        } else {
            stringQuery.append(
                "CREATE OR REPLACE VIEW dataset_" + datasetId + "." + "\"" + queryViewName + "\""
        ...
        while (iterator.hasNext()) {
            ...
            // id
            stringQuery.append(
                    "(select fv.id from dataset_" + datasetId + QUERY_FILTER_BY_ID_RECORD)
            ...
            switch (type) {
                case DATE:
                    stringQuery.append("(select case when dataset_" + datasetId
                            ...
                            + datasetId + QUERY_FILTER_BY_ID_RECORD).append(schemaId).append(AS)
                        .append("\"").append(columns.get(i).getName()).append("\" ");
                    break;
                case DATETIME:
                    stringQuery.append("(select case when dataset_" + datasetId
                ...
                case GEOMETRYCOLLECTION:
                    stringQuery.append(
                            "(select fv.geometry from dataset_" + datasetId + QUERY_FILTER_BY_ID_RECORD)
                        .append(schemaId).append(AS).append("\"").append(columns.get(i).getName())
                        .append("\" ");
                    break;
                default:
                    stringQuery.append(
                            "(select case when fv.value = '' then null else fv.value end from dataset_"
                                + datasetId + QUERY_FILTER_BY_ID_RECORD).append(schemaId).append(AS)
                        .append("\"").append(columns.get(i).getName()).append("\" ");
                    break;
            }
