...
public class JdbcRecordStoreServiceImpl implements RecordStoreService {
  ...
  public void distributeTables(Long datasetId) {
    ...
    try (Connection connection = dataSource.getConnection();
      ...
      LOG.error("Error reading commands file to distribute the dataset {}. {}", datasetId, e.getMessage());
      try {
  ...
  public void createEmptyDataSet(String datasetName, String idDatasetSchema)
    ...
    try (BufferedReader br =
      ...
      LOG.error("Error reading commands file to create the idDatasetSchema {}. {}", idDatasetSchema, e.getMessage());
  ...
  public void createDataSnapshot(Long idDataset, Long idSnapshot, Long idPartitionDataset,
      ...
      final DataSetMetabaseVO dataset = dataSetMetabaseControllerZuul.findDatasetMetabaseById(idDataset);
      final Long dataflowId = dataset.getDataflowId();
      final boolean isBigdata = dataflowControllerZuul.isBigDataflow(dataflowId);
      String type = SNAPSHOT;

      if (isBigdata) { // It's a bigData dataflow, connect to S3
          try {
              type = checkType(idDataset, idSnapshot);
              final long dataProviderId = dataset.getDataProviderId() != null ? dataset.getDataProviderId() : 0L;
              if (DatasetTypeEnum.COLLECTION.equals(dataset.getDatasetTypeEnum())) {  // copy to eu dataset
                  ...
                  final S3PathResolver dcPath = new S3PathResolver(dataflowId, idDataset, S3_TABLE_NAME_ROOT_DC_FOLDER_PATH);

                  final List<EUDatasetVO> euDatasets = euDatasetControllerZuul.findEUDatasetByDataflowId(dataflowId);
                  Long euDatasetId = 0L;
                  ...
                  final S3PathResolver euPath = new S3PathResolver(dataflowId, euDatasetId);
                  ...
                  final List<S3Object> tableNameFilenames = s3Helper.getFilenamesFromTableNames(dcPath);
                  final Long finalEuDatasetId = euDatasetId;
                  tableNameFilenames.forEach(file -> {
                      final String key = file.key();
                      final String filename = new File(key).getName();
                      if (!key.contains("/attachments/")) {
                          ...
                          try {
                              final String tableNameSnapshotPath = s3Service.getS3Path(euPath);
                          ...
                          final String extractedValue = key.replaceFirst(".*?/attachments/", "");
                          final String tableName = extractedValue.replaceFirst("/.*", "");
                          ;
                          final S3PathResolver s3AttachmentDCFolderPathResolver = new S3PathResolver(dataflowId, dataProviderId, dataset.getId(), tableName, filename, S3_ATTACHMENTS_DC_FOLDER_PATH);
                          final String attachmentDCPathInS3 = s3Service.getS3Path(s3AttachmentDCFolderPathResolver);
                          final List<ObjectIdentifier> attachmentsInDC = s3Helper.listObjectsInBucket(attachmentDCPathInS3);
                          for (ObjectIdentifier attachmentInDC : attachmentsInDC) {
                              final String attachmentPathInDC = attachmentInDC.key();
                              final String euDatasetFormattedFolder = s3Service.formatFolderName(finalEuDatasetId, S3_EU_DATASET_PATTERN);
                  ...
                  final S3PathResolver snapshotPath = new S3PathResolver(dataflowId, dataProviderId, idDataset);

                  //Get table name file from S3, save it locally and then upload to DC table name path
                  final S3PathResolver providerPath = new S3PathResolver(dataflowId, dataProviderId, idDataset);
                  providerPath.setPath(S3_PROVIDER_PATH);
                  LOG.info("Getting tableNameFilenames for path resolver {}", providerPath);
                  final List<S3Object> tableNameFilenames = s3Helper.getFilenamesFromTableNames(providerPath);
              ...
              LOG.error("Error creating snapshot for dataset {}, processId {}", idDataset, processId,
                      e);
              ...
              if (SNAPSHOT.equals(type)) {
                  final SnapshotVO snapshot = dataSetSnapshotControllerZuul.getById(idSnapshot);
                  final Map<String, Object> createSnapshot = new HashMap<>();
              ...
              if (SCHEMA.equals(type)) {
                  final Map<String, Object> createSchemaSnapshot = new HashMap<>();
      ...
      else { // It's a Citus dataflow, create a JDBC connection
          final ConnectionDataVO connectionDataVO = getConnectionDataForDataset(LiteralConstants.DATASET_PREFIX + idDataset);
          try (final Connection con = DriverManager.getConnection(connectionDataVO.getConnectionString(), connectionDataVO.getUser(), connectionDataVO.getPassword())) {
              type = checkType(idDataset, idSnapshot);
              ...
              else if (!DatasetTypeEnum.COLLECTION.equals(typeDataset) && Boolean.TRUE.equals(prefillingReference)) {
                  copyQueryRecord = "COPY (SELECT id, id_record_schema, id_table, " + idPartitionDataset
                          + ",data_provider_code FROM dataset_" + idDataset
              ...
              if (DatasetTypeEnum.DESIGN.equals(typeDataset) && !prefillingReference) {
  ...
  public void createSnapshotToClone(Long originDataset, Long targetDataset,
    ...
    try (Connection con = DriverManager.getConnection(connectionDataVO.getConnectionString(),
      ...
      LOG.error("Error creating the data from the origin dataset {}", originDataset, e);
    ...
    try (
      ...
      LOG.error("Error restoring the data into the target dataset {}", targetDataset, e);
    } catch (Exception e) {
  ...
  private void copyProcessSpecificFileSnapshot(Long datasetId, Long idSnapshot,
    ...
    for (Long i = startingNumber; i <= endingNumber; i++) {
      ...
      try {
        ...
        LOG.error("Error in recover copy field process for snapshotId {} with error {}", idSnapshot, e);
      }
    }
  ...
  public void restoreSpecificFileSnapshot(Long datasetId, Long idSnapshot,
    ...
    try {
      ...
      LOG.error("Error in method restoreSpecificFileSnapshot for datasetId: {} with error {}",
              datasetId, e);
    }
  }
  ...
  public void updateMaterializedQueryView(Long datasetId, String user, Boolean released,
    ...
    try {
      ...
      LOG.error("Error updating Materialized view for datasetId {} : {}", datasetId, e.getMessage(), e);
    }
      ...
      if (!Boolean.TRUE.equals(datasetControllerZuul.getCheckView(id))) {
        ...
        for (String view : viewList) {
          try {
            ...
            LOG.error("Error refreshing materialized view from dataset {}", id);
          }
        }
    ...
    if (!CollectionUtils.isEmpty(nameFiles)) {
      nameFiles.stream().forEach(f -> {
        ...
        try (Stream<String> lines = Files.lines(pathFile)) {
          ...
          LOG.error("Error modifying the file {} during the data copy in cloning process for datasetId {}", f, datasetId);
        } catch (Exception e) {
    ...
    if (!CollectionUtils.isEmpty(fileNames)) {
      fileNames.stream().forEach(f -> {
        try {
          ...
          LOG.error("Error deleting the file {} during the data copy in cloning process", f);
        }
      });
    }
  }
  ...
  private void restoreSnapshot(Long datasetId, Long idSnapshot, Long partitionId,
    ...
    try (
      ...
      if (!prefillingReference) {
        ...
        LOG.error("Error restoring the snapshot data due to error {}.", e.getMessage(), e);
  ...
  private void restoreSnapshotDL(Long datasetId, Long idSnapshot, Long partitionId,
    ...
    try {
      ...
      try {
        ...
        LOG.error("error processing json for releaseTaskVO {}", releaseTaskVO);
        throw e;
      }
              ...
              .forEach(table -> {
                try {
                  if (DatasetTypeEnum.REPORTING.equals(datasetType) && RELEASE_COMPLETED_EVENT.equals(successEventType)) { //release
                    ...
                    //delete old attachments of provider
                    S3PathResolver dataCollectionProviderAttachmentsPath = new S3PathResolver(dataflowId, providerId, datasetId, nameTableSchema);
                    dataCollectionProviderAttachmentsPath.setPath(S3_ATTACHMENTS_DC_PROVIDER_PATH);
                    if (s3Helper.checkTableNameDCProviderFolderExist(dataCollectionProviderAttachmentsPath)) {
                      s3Helper.deleteTableNameDCFolder(dataCollectionProviderAttachmentsPath);
                      LOG.info("Successfully deleted files in path: {}", dataCollectionProviderAttachmentsPath);
                    }
                    ...
                    tableNameFilenames.stream()
                            ...
                            .forEach(file -> {
                              ...
                              try {
                                ...
                                LOG.error("Error in getFileFromS3 process for reportingDatasetId {}, dataflowId {}", datasetId, dataflowId, e);
                              }
                            });
                    ...
                    LOG.info("EUDATASET dataset {}", datasetId);
                  ...
                  LOG.error("Error in delete and copy release process for finalJobId {}, dataflowId {}",
  ...
  private void addNewValidationsToDC(Long datasetId, Long dataflowId, Long providerId, ProcessVO finalProcessVO, Long finalJobId) {
    S3PathResolver dataCollectionValidationTablePath = new S3PathResolver(dataflowId, providerId, datasetId);
    dataCollectionValidationTablePath.setPath(S3_TABLE_NAME_VALIDATE_DC_PATH);
    List<S3Object> validationTableNameFileNames = getListOfS3Files(finalProcessVO.getDatasetId(), dataflowId, jobControllerZuul.findProviderIdById(finalJobId), S3_VALIDATION_TABLE_PATH);
    validationTableNameFileNames.forEach(s3Object -> {

      String key = s3Object.key();
      String filename = new File(key).getName();
      dataCollectionValidationTablePath.setFilename(filename);
      dataCollectionValidationTablePath.setParquetFolder(key.split("/")[5]);
      try {
        LOG.info("Getting validation file from S3 with key : {} and filename : {}", key, filename);
        File parquetFile = s3Helper.getFileFromS3(key, filename, pathSnapshot, LiteralConstants.PARQUET_TYPE);
        String tableNameDCPath = s3Service.getS3Path(dataCollectionValidationTablePath);
        LOG.info("Uploading validation file to bucket parquetFile path : {} in path: {}", tableNameDCPath, parquetFile.getPath());
        s3Helper.uploadFileToBucket(tableNameDCPath, parquetFile.getPath());
        LOG.info("Uploading validation finished successfully for {}", tableNameDCPath);
      } catch (IOException e) {
        LOG.error("Error in getFileFromS3 process for dataflowId {}", dataflowId, e);
      }
    });
  }

  private void deletePreviousValidationsFromDC(Long datasetId, Long dataflowId, Long providerId) {
    S3PathResolver dataCollectionValidationPath = new S3PathResolver(dataflowId, providerId, datasetId);
    dataCollectionValidationPath.setPath(S3_VALIDATION_DC_PATH);
    if (s3Helper.checkTableNameDCProviderFolderExist(dataCollectionValidationPath)) {
      s3Helper.deleteTableNameDCFolder(dataCollectionValidationPath);
      LOG.info("Successfully deleted validation files in path: {}", dataCollectionValidationPath);
    }
  }

  private List<S3Object> getListOfS3Files(Long datasetId, Long dataflowId, Long dataProviderCode, String path) {
    S3PathResolver s3Path = new S3PathResolver(dataflowId, dataProviderCode, datasetId);
    s3Path.setPath(path);
  ...
  private void copyProcess(Long dataflowId, Long dataCollectionId, Long idSnapshot, DatasetTypeEnum datasetType,
    ...
    try {
      ...
      if (snapFileForSplitting.isForSplitting() == true) {
        ...
        for (int i = 1; i <= snapFileForSplitting.getNumberOfFiles(); i++) {
          ...
          try {
            ...
            LOG.error("Error in copy field process for snapshotId {} with error", idSnapshot, e);
            throw e;
          }
        }
      } else {
      ...
      LOG.error("Unexpected error! Error in copyProcess for dataCollectionId {} and snapshotId {}. Message: {}", dataCollectionId, idSnapshot, e.getMessage());
      throw e;
    }
  }
  ...
  private void copyProcessSpecificFileSnapshot(Long datasetId, Long idSnapshot,
    ...
    for (int i = startingNumber; i <= endingNumber; i++) {
      ...
      try {
        ...
        LOG.error("Error in copy field process for snapshotId {} with error",
                idSnapshot, e);
        throw e;
      }
    }
  ...
  private void copyFromFile(String query, String fileName, CopyManager copyManager)
    ...
    try (FileReader from = new FileReader(path.toString())) {
      ...
      LOG.error("Unexpected error! Error copying from file {} with query {} Message: {}", fileName, query, e.getMessage());
      throw e;
    }
  }
  ...
  private void copyFromFileRecovery(String query, String fileName, CopyManager copyManager)
    ...
    try {
      ...
      LOG.error("Error in recover copy field process for fileName {} with error", fileName, e);
      throw e;
    }
  }
    ...
    try {
      ...
      for (int j = 1; j <= numberOfFiles; j++) {
        ...
        try {
          ...
          LOG.error("error processing json for snap file {}", splitFileName);
          throw e;
        }
        TaskType taskType = null;
      ...
      LOG.error("Error in file {} with error", inputfile, e);
    }
