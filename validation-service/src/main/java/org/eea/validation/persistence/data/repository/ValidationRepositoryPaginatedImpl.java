...
public class ValidationRepositoryPaginatedImpl implements ValidationRepositoryPaginated {
  ...
  public List<GroupValidationVO> findGroupRecordsByFilter(Long datasetId,
    ...
    String basicQuery = String.format(
        "select v.message as message, v.id_rule as idRule, v.level_error as levelError, v.type_entity as typeEntity,v.table_name as originName, count(*) as numberOfRecords from dataset_%s.Validation v  where v.id is not null ",
        datasetId);
    String orderPart = "";
    orderPart = addOrderBy(headerField, asc, orderPart);
    ...
    String groupBy = "group by v.message,v.level_error, v.id_rule, v.type_entity,v.table_name ";
    String page =
