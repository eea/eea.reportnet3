...
export class Calendar extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!this.props.onViewDateChange && !this.viewStateChanged) {
      ...
      if (
        ...
        (propValue && propValue instanceof Date && propValue.getTime() !== prevPropValue.getTime())
      ) {
        let viewDate =
          this.props.viewDate && this.isValidDate(this.props.viewDate)
            ? this.props.viewDate
    ...
    if (
      (prevProps.value !== this.props.value && (!this.viewStateChanged || !this.state.overlayVisible)) ||
  ...
  isValidSelection(value) {
    ...
    if (this.isSingleSelection()) {
      if (
        !(
          this.isSelectable(value.getDate(), value.getMonth(), value.getFullYear(), false) &&
      ...
      value.every(
        v => this.isSelectable(v.getDate(), v.getMonth(), v.getFullYear(), false) && this.isSelectableTime(value)
      )
    ) {
  ...
  updateFocus() {
    ...
    if (this.navigation) {
      if (this.navigation.button) {
        ...
        if (this.navigation.backward) DomHandler.findSingle(this.overlayRef.current, '.p-datepicker-prev').focus();
        else DomHandler.findSingle(this.overlayRef.current, '.p-datepicker-next').focus();
      } else {
  ...
  initFocusableCell() {
    ...
    if (this.view === 'month') {
      ...
      if (!cell) {
        ...
        else cell = DomHandler.findSingle(this.overlayRef.current, '.p-datepicker-calendar td span:not(.p-disabled)');
      }
    }
  ...
  validateDate(value) {
    if (this.props.yearNavigator) {
      ...
      const minYear =
        this.props.minDate && minRangeYear != null
          ? Math.max(this.props.minDate.getFullYear(), minRangeYear)
          : this.props.minDate || minRangeYear;
      const maxYear =
        this.props.maxDate && maxRangeYear != null
          ? Math.min(this.props.maxDate.getFullYear(), maxRangeYear)
    ...
    if (this.props.monthNavigator && this.props.view !== 'month') {
      ...
      let viewMonthWithMinMax = parseInt(
        (this.isInMinYear(value) && Math.max(this.props.minDate.getMonth(), viewMonth).toString()) ||
          (this.isInMaxYear(value) && Math.min(this.props.maxDate.getMonth(), viewMonth).toString()) ||
          viewMonth
      );
  ...
  onDateSelect(event, dateMeta, timeMeta) {
    ...
    DomHandler.find(this.overlayRef.current, '.p-datepicker-calendar td span:not(.p-disabled)').forEach(
      ...
      if (this.isSingleSelection()) {
        ...
        if (this.props.value[1])
          return (
            ...
            this.isDateBetween(this.props.value[0], this.props.value[1], dateMeta)
          );
        else {
  ...
  renderBackwardNavigator(isVisible) {
    let navigatorProps = isVisible
      ? { onClick: this.onPrevButtonClick, onKeyDown: e => this.onContainerButtonKeydown(e) }
  ...
  renderForwardNavigator(isVisible) {
    let navigatorProps = isVisible
      ? { onClick: this.onNextButtonClick, onKeyDown: e => this.onContainerButtonKeydown(e) }
      : { style: { visibility: 'hidden' } };
    return (
      ...
      return (
        <select className="p-datepicker-month" onChange={this.onMonthDropdownChange} value={viewMonth}>
          {monthNames.map((month, index) => {
            if (
              (!this.isInMinYear(viewDate) || index >= this.props.minDate.getMonth()) &&
              (!this.isInMaxYear(viewDate) || index <= this.props.maxDate.getMonth())
            ) {
              return (
  ...
  renderDates(monthMetaData, groupIndex) {
    return monthMetaData.dates.map((weekDates, index) => {
      return <tr key={index}>{this.renderWeek(weekDates, monthMetaData.weekNumbers[index], groupIndex)}</tr>;
    });
  }
  ...
  render() {
    const className = classNames('p-calendar p-component p-inputwrapper', this.props.className, {
      ...
      'p-inputwrapper-filled':
        this.props.value ||
        (DomHandler.hasClass(this.inputRef.current, 'p-filled') && this.inputRef.current.value !== ''),
