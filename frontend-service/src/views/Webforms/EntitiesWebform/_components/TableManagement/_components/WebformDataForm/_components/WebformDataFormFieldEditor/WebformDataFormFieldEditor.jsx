...
export const WebformDataFormFieldEditor = ({
  ...
  const renderMultiselectCodelist = (field, fieldValue) => {
    if (TextUtils.areEquals(field, 'listofsingleentities') && hasSingle && !isEmpty(fieldValue)) {
      onChangeForm(field, []);
    }

    const options = column.codelistItems.map(codelistItem => ({
      itemType: codelistItem,
      value: TextUtils.areEquals(field, 'listofsingleentities') ? getId(codelistItem) : codelistItem
    }));

    return (
      <MultiSelect
        appendTo={document.body}
        disabled={TextUtils.areEquals(field, 'listofsingleentities') && hasSingle}
