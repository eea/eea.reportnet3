      ...
      {!isNil(onDeleteClick) && !hideDeletion && (
        <Button
          className={`${`p-button-rounded p-button-secondary-transparent ${styles.deleteRowButton}`} ${
            isDeletingDocument || isDeletingDatasetData || (rowUpdatingId === rowDataId && isUpdating) || disabledButtons
              ? null
              : 'p-button-animated-blink'
          }`}
          disabled={isDeletingDocument || isDeletingDatasetData || (rowUpdatingId === rowDataId && isUpdating) || disabledButtons}
          icon={rowDeletingId !== rowDataId || !isDeletingDatasetData || !isDeletingDocument ? 'trash' : 'spinnerAnimate'}
