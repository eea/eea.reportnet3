...
export const SystemNotificationService = {
  all: async () => {
    ...
    notifications.userNotifications = systemNotificationsDTO?.data?.map(systemNotificationDTO => {
      const { id, message, enabled, type } = systemNotificationDTO;
      return new SystemNotification({ id, message, enabled, type, lifeTime: 5000 });
    });

    notifications.totalRecords = systemNotificationsDTO.data.totalRecords;
