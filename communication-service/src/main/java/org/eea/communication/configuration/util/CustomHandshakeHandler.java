 ...
 *
 * <p>
 * Performs initial validation of the WebSocket handshake request - possibly rejecting it through
 ...
 * {@link org.springframework.web.socket.server.RequestUpgradeStrategy}, which will update the
 * response as necessary and initialize the WebSocket. Currently supported servers are Jetty
 * 9.0-9.3, Tomcat 7.0.47+ and 8.x, Undertow 1.0-1.3, GlassFish 4.1+, WebLogic 12.1.3+.
 *
...
public class CustomHandshakeHandler implements HandshakeHandler, Lifecycle {
  ...
  private static RequestUpgradeStrategy initRequestUpgradeStrategy() {
    ...
    try {
      ...
      throw new IllegalStateException("Failed to instantiate RequestUpgradeStrategy: " + className,
          ex);
    }
  }
   ...
   * {@link SubProtocolCapable} then there is not need to explicitly configure this property. That
   * is certainly the case with the built-in STOMP over WebSocket support. Therefore this property
   * should be configured explicitly only if the WebSocketHandler does not implement
  ...
  protected void handleInvalidConnectHeader(ServerHttpRequest request, ServerHttpResponse response)
      throws IOException {
    if (logger.isErrorEnabled()) {
      logger.error("Handshake failed due to invalid Connection header "
  ...
  protected void handleWebSocketVersionNotSupported(ServerHttpRequest request,
   ...
   * <p>
   * As of 4.1, the default implementation of this method filters the list to leave only extensions
   * that are both requested and supported.
   * 
   ...
   * <p>
   * Subclasses can provide custom logic for associating a user with a session, for example for
