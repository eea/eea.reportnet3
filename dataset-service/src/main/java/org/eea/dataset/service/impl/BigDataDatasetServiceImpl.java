...
public class BigDataDatasetServiceImpl implements BigDataDatasetService {
    ...
    public void importBigData(Long datasetId, Long dataflowId, Long providerId, String tableSchemaId,
                              ...
                              String fmeJobId) throws Exception {
        ...
        try {
            ...
            if(file == null){
                ...
                if(preSignedURL.endsWith(".csv")) {
                    s3File = s3Helper.getFileFromS3(preSignedURL, filePathStructure.replace(".csv", ""), importPath, LiteralConstants.CSV_TYPE);
                    fileName = s3File.getName();
                }
                else if(preSignedURL.endsWith(".zip")){
                    s3File = s3Helper.getFileFromS3(preSignedURL, filePathStructure.replace(".zip", ""), importPath, LiteralConstants.ZIP_TYPE);
                    fileName = s3File.getName();
                }
