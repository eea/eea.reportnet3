...
public class DataLakeDataRetrieverServiceImpl implements DataLakeDataRetrieverService {
    ...
    public TableVO getTableValuesDLById(final Long datasetId, final String idTableSchema,
        ...
        if (COLLECTION.equals(dataset.getDatasetTypeEnum())) {
            ...
            LOG.info("dremioHelperService.checkFolderPromoted(s3PathResolver) : {}",  dremioHelperService.checkFolderPromoted(s3PathResolver, S3_TABLE_NAME_ROOT_DC_FOLDER_PATH, false));
            if (s3Helper.checkTableNameDCFolderExist(s3PathResolver) && dremioHelperService.checkFolderPromoted(s3PathResolver, S3_TABLE_NAME_ROOT_DC_FOLDER_PATH, false)) {
