...
public class FileTreatmentHelper implements DisposableBean {
    ...
    private Map<String, Object> processesMap;
    ...
    private void init() {
        ...
        importExecutorService.submit(() -> LOG.info("initializer import executor service"));
    ...
    public void createReferenceDatasetFiles(DataSetMetabase dataset) throws IOException {
        ...
        try {
            ...
            LOG.error("File not created in dataflow {}. Message: {}", dataset.getDataflowId(),
                    e.getMessage(), e);
        }
    ...
    public void exportFile(Long datasetId, String mimeType, String tableSchemaId, String tableName,
        ...
        try {
            ...
            LOG.error("Error exporting table data from dataset Id {} with schema {}.", datasetId,
                    tableSchemaId);
    ...
    public void exportFileDL(Long datasetId, String mimeType, String tableSchemaId, String tableName,
        ...
        try {
            ...
            if (filtered) {
                ...
                convertParquetFile(datasetId, mimeType, tableSchemaId, tableName, false);
            }
    ...
    public void convertParquetFile(Long datasetId, String mimeType, String tableSchemaId, String tableName, Boolean includeRecordId) {
        DataSetMetabaseVO dataset = datasetMetabaseService.findDatasetMetabase(datasetId);
        ...
        try {
            ...
            if (mimeType.equalsIgnoreCase(CSV.getValue())) {
                convertParquetToCSV(exportFilenames, tableName, datasetId, tableSchemaId, dataset.getDatasetTypeEnum(), includeRecordId);
    ...
    private void convertParquetFileZip(Long datasetId, String mimeType, String tableName, ZipOutputStream out, String tableSchemaId, DatasetTypeEnum datasetTypeEnum, Boolean includeRecordId) {
        DataSetMetabaseVO dataset = datasetMetabaseService.findDatasetMetabase(datasetId);
        S3PathResolver s3PathResolver = new S3PathResolver(dataset.getDataflowId(), tableName);
        ...
        try {
            ...
            if (mimeType.equalsIgnoreCase(CSV.getValue())) {
                convertParquetToCSVinZIP(exportFilenames, tableName, datasetId, tableSchemaId, out, datasetTypeEnum, includeRecordId);
            } /*else if (mimeType.equalsIgnoreCase(FileTypeEnum.XLSX.getValue())) {
                File parquetFile = s3Helper.getFileFromS3(key, nameDataset, exportDLPath, LiteralConstants.PARQUET_TYPE);
                nameDataset = nameDataset + XLSX_TYPE;
                File xlsxFile = new File(exportDLPath + nameDataset);
    ...
    public List<TableSchema> getTables(Long datasetId) {
        ...
        try {
            ...
            LOG.error("Error finding datasetSchema by Id. DatasetSchemaId {}. Message {}",
    ...
    public void exportDatasetFile(Long datasetId, String mimeType) {
        ...
        try {
            ...
            try {
                ...
                LOG.error("Error sending export dataset fail notification for datasetId {}. Message {}",
    ...
    /**
     * Release lock.
     * Also Deletes The Import Directory
     * @param datasetId the dataset id
     */
    public void releaseLockAndDeleteImportFileDirectory(Long datasetId) {
        try {
            Map<String, Object> importFileData = new HashMap<>();
            importFileData.put(LiteralConstants.SIGNATURE, LockSignature.IMPORT_FILE_DATA.getValue());
            importFileData.put(LiteralConstants.DATASETID, datasetId);
            ...
            if (null != integrationVO) {
                ...
                importExecutorService.submit(() -> {
                    try {
                        ...
                        LOG.error("RN3-Import: Unexpected error in queueImportProcess for datasetId {} and tableSchemaId {}. {}", datasetId, tableSchemaId, e.getMessage(), e);
      ...
      throws InterruptedException {
            ...
            for (File file : files) {
                ...
                try (InputStream inputStream = new FileInputStream(file)) {
                    ...
                    LOG.error("RN3-Import file failed: fileName={}, tableSchemaId={}", fileName,
                            tableSchemaId, e);
                    error = e.getMessage();
                } catch (Exception e) {
    ...
    void addImportTaskToProcess(String filePath, Long partitionId,String idTableSchema,Long dataflowId,Long datasetId, String fileName,boolean replace,
        ...
        if(true){
            ...
            value.put("partitionId",partitionId.toString());
            ...
            value.put("startLine",startLine.toString());
            value.put("endLine",endLine.toString());
    ...
    private void createFilesAndZipDL(DataSetMetabase dataset, String zipDestinationPath, String nameFileUnique) throws EEAException {
        ...
        private void convertParquetToCSVinZIP(List<S3Object> exportFilenames, String tableName, Long datasetId, String tableSchemaId, ZipOutputStream out, DatasetTypeEnum datasetTypeEnum, Boolean includeRecordId) {
            try {
                File csvFile;
                ...
                if (CollectionUtils.isEmpty(exportFilenames)) {
                    ...
                    csvFile = s3ConvertService.createCSVFile(exportFilenames, tableName, datasetId, datasetTypeEnum, headers, includeRecordId);
                }
        ...
        private void convertParquetToCSV(List<S3Object> exportFilenames, String tableName, Long datasetId, String tableSchemaId, DatasetTypeEnum datasetTypeEnum, Boolean includeRecordId) {
            try {
                List<String> headers = getFieldsFromSchema(datasetId, tableSchemaId);
                if (CollectionUtils.isEmpty(exportFilenames)) {
                    s3ConvertService.createEmptyCSVFile(tableName, datasetId, headers);
                } else {
                    s3ConvertService.createCSVFile(exportFilenames, tableName, datasetId, datasetTypeEnum, headers, includeRecordId);
                }
            } catch (Exception e) {
