...
public class IcebergToParquetConversionCommand extends AbstractEEAEventHandlerCommand {
  ...
  public void execute(EEAEventVO eeaEventVO) throws EEAException {
    ...
    try {
      bigDataDatasetService.convertIcebergToParquetTables(datasetId, preparationCode, dataflowId, providerId, tableSchemaIds, user, lockValue);
    }
    catch (Exception e){
      LOG.error("Could not call async method convertIcebergToParquetTables for datasetId {} preparationCode {} Error: {}", datasetId, preparationCode, e.getMessage());
