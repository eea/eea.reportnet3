    ...
    stages {
       ...
       when {
        ...
        stage('Push to EEA GitHub') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'jenkins-eea-altia', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    ...
                    sh('git pull https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/eea/eea.reportnet3.git hotfix/v3.0.4.1 --allow-unrelated-histories')
                    sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/eea/eea.reportnet3.git HEAD:hotfix/v3.0.4.1')
                }
            }
        }
        ...
        stage('Build Docker Images') {
            when {
                expression {
                   return BRANCH_NAME == "develop" || BRANCH_NAME == "v3.0.4.1-RC1"
                }
            }
            parallel {
