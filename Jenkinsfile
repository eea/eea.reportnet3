        ...
        stage('Static Code Analysis') {
            steps {
                withSonarQubeEnv('Altia SonarQube') {
                    ...
                    sh 'mvn org.sonarsource.scanner.maven:sonar-maven-plugin:3.2:sonar -Dsonar.projectKey=org.eea:reportnet:' + env.BRANCH_NAME.replace('/', '_') + ' -Dsonar.projectName=ReportNet3-' + env.BRANCH_NAME + ' -Dsonar.branch.name=' + env.BRANCH_NAME.replace('/', '_')
                }
