...
pipeline {
    ...
    stages {
        ...
        stage('Push to EEA GitHub') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'jenkins-eea-altia', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    ...
                    sh('git pull https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/eea/eea.reportnet3.git hotfix/v3.5.4.0 --allow-unrelated-histories')
                    sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/eea/eea.reportnet3.git HEAD:hotfix/v3.5.4.0')
                }
            }
        }
        ...
        stage('Build Docker Images') {
            when {
                expression {
                   return BRANCH_NAME == "release/v3.5.4.0-RC1" 
                }
            }
            parallel {
