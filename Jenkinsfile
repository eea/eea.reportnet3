...
pipeline {
    ...
    stages {
        ...
        stage('Push to EEA GitHub') {
            when {
                expression {
                   return BRANCH_NAME == "develop" || BRANCH_NAME == "release/3.0.0-RC.1.1" || BRANCH_NAME == "release/3.0.0-RC.2.0"
                }
            }
            steps {
        ...
        stage('Build Docker Images') {
            when {
                expression {
                   return BRANCH_NAME == "develop" || BRANCH_NAME == "sandbox" ||  BRANCH_NAME == "release/3.0.0-RC.1.1" || BRANCH_NAME == "release/3.0.0-RC.2.0"
                }
            }
            parallel {
